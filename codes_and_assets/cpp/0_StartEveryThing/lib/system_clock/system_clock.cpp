...
namespace {
void inline __clk_bad_init_hook() {
  while (1)
    ;
}
/**
 ...
 */
static constexpr const auto DEFAULT_RCC_PLL_VALUE = RCC_PLL_MUL9;

} // namespace

namespace system_clock {

uint32_t provide_application_sysfreq() { return HAL_RCC_GetSysClockFreq(); }
...
void system_delay_us(uint32_t us) {
  ...
  uint32_t prevValue, currentValue,
      elapsedTicks =
          0; // Variables for SysTick counter values and elapsed ticks
  ...
  ticks = us * sysclk_freq; // Calculate the number of ticks needed for the
                            // desired delay in microseconds
  ...
  while (1) {
    ...
    if (currentValue != prevValue) {
      if (currentValue < prevValue) {
        elapsedTicks +=
            prevValue - currentValue; // Count the elapsed ticks (SysTick is a
                                      // decrementing counter)
      } else {
        elapsedTicks +=
            reloadValue - currentValue + prevValue; // Handle the rollover case
      }
      prevValue =
          currentValue; // Update the previous value to the current value

      if (elapsedTicks >= ticks) // If the elapsed time is greater than or equal
 ...
 *
 * @param plln PLL multiplication factor (determines final system clock
 * frequency)
 * @return HAL_StatusTypeDef HAL_OK if successful, HAL_ERROR otherwise
 */
static void __init_as_clk_72MHz(uint32_t plln) {
  HAL_StatusTypeDef ret =
      HAL_ERROR; /* Initialize return status to error state */
  RCC_OscInitTypeDef rcc_osc_init = {
      0}; /* Initialize oscillator structure with zeros */
  RCC_ClkInitTypeDef rcc_clk_init = {
      0}; /* Initialize clock structure with zeros */

  /* Configure the High Speed External (HSE) oscillator and Phase Locked Loop
   * (PLL) */
  rcc_osc_init.OscillatorType =
      RCC_OSCILLATORTYPE_HSE;         /* Select HSE as oscillator type */
  rcc_osc_init.HSEState = RCC_HSE_ON; /* Enable HSE oscillator */
  rcc_osc_init.HSEPredivValue =
      RCC_HSE_PREDIV_DIV1; /* Set HSE prescaler to 1 (no division) */
  rcc_osc_init.PLL.PLLState = RCC_PLL_ON; /* Enable PLL */
  rcc_osc_init.PLL.PLLSource =
      RCC_PLLSOURCE_HSE;                  /* Select HSE as PLL input source */
  ...
  /**
   * @note Critical section: If oscillator configuration fails, the system
   * cannot proceed as clock settings are fundamental to all microcontroller
   * operations. The infinite loop should be replaced with proper error handling
   * in production code.
   */
  if (ret != HAL_OK) {
  ...
  rcc_clk_init.SYSCLKSource =
      RCC_SYSCLKSOURCE_PLLCLK; /* Use PLL output as system clock */
  rcc_clk_init.AHBCLKDivider = RCC_SYSCLK_DIV1; /* AHB bus clock = SYSCLK/1 */
  rcc_clk_init.APB1CLKDivider =
      RCC_HCLK_DIV2; /* APB1 (low-speed) bus clock = HCLK/2 */
  rcc_clk_init.APB2CLKDivider =
      RCC_HCLK_DIV1; /* APB2 (high-speed) bus clock = HCLK/1 */
...
// clock init
void system_clock_init() { __init_as_clk_72MHz(DEFAULT_RCC_PLL_VALUE); }
